...
//Start API calls
jsonDataCall(versions_url, (json) => {
    console.log('Gitlab Version: ', json);

    //Call main projects endpoint
    jsonDataCall(projects_url, (projects) => {
        handleProjectData(projects_url,'output/projects.json', projects); //TODO args for file path
    });

    jsonDataCall(subgroups_url, (sub_groups) => {
        sub_groups.forEach(group => {
            ...
            jsonDataCall(url, (projects) => {
                console.log('\nGroup:' + group.path);
                handleProjectData(url, `output/projects-${group.path}.json`, projects); //TODO args for file path
            });
        })
    });
});
